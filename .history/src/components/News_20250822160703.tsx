...
const News: React.FC = () => {
  ...
  const handleBookmarkClick =(article)=>{
    setBookmark((prevBookmark=>{
      const updatedBookmark = prevBookmark.find((bookmark)=> bookmark.title === article.title) ? prevBookmark.filter((bookmark)=> bookmark.title !== article.title) : [...prevBookmark,article]
    }))
  }
    ...
    <div className="text-regular h-full w-full flex flex-col justify-between gap-y-8">
      ...
      <div className="h-[calc(100%-16rem)]  flex gap-x-8 px-8">
        ...
        <Modal show={showModal} article={selecetedArticle} onClose={()=> setShowModal(false)} />
          <BookMark show={showBookmark} onSelectedArticle={handleArticleClick} onClose={()=> setShowBookmark(false)}/>
        <div
